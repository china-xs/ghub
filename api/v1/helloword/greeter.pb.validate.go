...
func (m *CreateGreeterRequest) validate(all bool) error {
	...
	if l := len(m.GetName()); l < 100 || l > 256 {
		err := CreateGreeterRequestValidationError{
			field:  "Name",
			reason: "value length must be between 100 and 256 bytes, inclusive",
		}
		if !all {
			return err
		}
	...
	if len(m.GetRoleId()) > 0 {

		if l := len(m.GetRoleId()); l < 1 || l > 100 {
			err := CreateGreeterRequestValidationError{
				field:  "RoleId",
		...
		_CreateGreeterRequest_RoleId_Unique := make(map[int32]struct{}, len(m.GetRoleId()))

		for idx, item := range m.GetRoleId() {
			_, _ = idx, item

			if _, exists := _CreateGreeterRequest_RoleId_Unique[item]; exists {
				err := CreateGreeterRequestValidationError{
					field:  fmt.Sprintf("RoleId[%v]", idx),
				...
				_CreateGreeterRequest_RoleId_Unique[item] = struct{}{}
			}

			if item <= 30 {
				err := CreateGreeterRequestValidationError{
					field:  fmt.Sprintf("RoleId[%v]", idx),
					reason: "value must be greater than 30",
				}
				if !all {
					return err
				}
